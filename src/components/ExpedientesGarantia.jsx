    ...
    <form className="app-dialog-panel" onSubmit={guardar}>
      ...
      <div className="app-dialog-body space-y-4">
        ...
        <fieldset disabled={ocupado} className="grid min-w-0 grid-cols-1 gap-3 sm:grid-cols-2">
          {[['referencia', 'N° referencia'], ['serie', 'Serie'], ['cliente', 'Cliente']].map(([campo, titulo]) => <label key={campo}>{titulo}<input className="control-field mt-1" required value={form[campo]} readOnly={Boolean(inicial.id)} onChange={e => cambiar(campo, e.target.value)}/></label>)}
...
export default function ExpedientesGarantia({ datos, tickets = [], ticketInicial, onCerrar }) {
  ...
  const nuevas = tickets.filter(t => { const n = nuevoExpediente(t); return n.serie && registros.some(r => r.serie === n.serie) && !registros.some(r => r.referencia === n.referencia) }).filter((t,i,a) => a.findIndex(x => String(x['N° REFERENCIA']) === String(t['N° REFERENCIA'])) === i)
  return <section className="space-y-4">
    ...
    {estado === 'listo' && nuevas.length > 0 && <div className="card space-y-2 border-2 border-amber-500 p-4"><h3 className="font-bold">Series con antecedentes en otros reportes</h3>{nuevas.map(t => <div key={t['N° REFERENCIA']} className="flex flex-wrap items-center gap-2 border-t border-slate-400 pt-2"><span className="break-all text-sm">#{t['N° REFERENCIA']} · {t.CLIENTE} · Serie {nuevoExpediente(t).serie}</span><button className="btn-ghost" onClick={() => setEditor(nuevoExpediente(t))}>Crear ficha de esta atención</button></div>)}</div>}
