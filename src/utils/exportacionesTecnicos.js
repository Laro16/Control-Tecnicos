...
export function clasificarGarantiaExportada(garantia) {
  if (garantia.vencida) return { prioridad: 0, estado: 'VENCIDA', accion: 'No atender sin garantía. Plazo vencido.' }
  if (garantia.tipoIncorrecto) return {
    ...
    accion: garantia.sinDatosSerie
      ...
      : 'No atender sin garantía. La serie está en plazo, pero el origen marcó Normal; revisar el motivo.',
  }
  if (garantia.sinDatosSerie) return { prioridad: 2, estado: 'VERIFICAR SERIE', accion: 'No se pudo calcular la vigencia. Verificar la serie antes de atender.' }
  return { prioridad: 3, estado: 'VIGENTE', accion: 'Dentro del plazo calculado según la serie y el catálogo.' }
}
...
export function prepararGarantiasExportacion(alertas = []) {
  return alertas.map(({ ticket, garantia }) => {
    ...
    if (garantia.fechaVerificada) diagnostico.accion = `Vencimiento real confirmado manualmente: ${garantia.vencDisplay}. ${garantia.vencida ? 'Plazo confirmado vencido.' : garantia.tipoIncorrecto ? 'Cobertura vigente confirmada; revisar el TIPO Normal original del Excel.' : 'Cobertura vigente según fecha confirmada.'}`
...
const columnasBase = [
  ...
  ['DIRECCIÓN', 55], ['TELÉFONO', 21], ['TIEMPO TRANSCURRIDO', 23, 'decimal'],
...
export async function crearLibroTecnicos(informe) {
  ...
  for (const hoja of informe.hojas) {
    ...
    if (hoja.nombre === 'Garantías' && hoja.filas.length) {
      const colores = [['VENCIDA', 'FFBE123C', 'FFFFE4E6'], ['NORMAL - REVISAR', 'FF6D28D9', 'FFEDE9FE'], ['VERIFICAR SERIE', 'FF92400E', 'FFFEF3C7'], ['VIGENTE', 'FF047857', 'FFD1FAE5']]
  ...
  resumen.getCell('A3').value = 'Fecha de evaluación'; resumen.getCell('B3').value = fechaExcel(informe.generado); resumen.getCell('B3').numFmt = 'dd/mm/yyyy'
  resumen.getCell('C3').value = 'Corte fijo: los días de garantía se calculan con esta fecha, no con el día en que abras el archivo.'
  ...
  for (const estado of ['VENCIDA', 'NORMAL - REVISAR', 'VERIFICAR SERIE', 'VIGENTE']) {
    resumen.getRow(fila++).values = [`Garantías: ${estado}`, { formula: `COUNTIF('Garantías'!F6:F${finGarantias},"${estado}")`, result: informe.garantias.filter(g => g.estado === estado).length }, estado === 'VENCIDA' ? 'Incluye TIPO Normal con plazo vencido. No se cuenta nuevamente como Normal por revisar.' : estado === 'NORMAL - REVISAR' ? 'TIPO Normal siempre es alerta, aunque la serie esté dentro del plazo.' : 'Clasificación según las mismas reglas del panel.']
  }
  ...
  resumen.getRow(fila + 3).values = ['Días al vencimiento', null, 'Número negativo = días vencida. Cero = vence en la fecha de evaluación. TIPO Normal no autoriza atención.']
